        ...
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('staff.events.index') }}">Events</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('staff.events.show', $event) }}">{{ $event->title }}</a></li>
                    <li class="breadcrumb-item active">Attendees</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('staff.events.show', $event) }}" class="btn btn-outline-secondary">
    ...
    <ul class="nav nav-tabs mb-4" id="attendeeTabs" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="attendees-tab" data-bs-toggle="tab" data-bs-target="#attendees" type="button" role="tab">
                <i class="bi bi-people me-2" style="color:#20798c;"></i>Attendees ({{ $regularAttendees->count() }})
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="volunteers-tab" data-bs-toggle="tab" data-bs-target="#volunteers" type="button" role="tab">
                <i class="bi bi-hand-thumbs-up me-2" style="color:#20798c;"></i>Volunteers ({{ $approvedVolunteers->count() }})
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link position-relative" id="pending-tab" data-bs-toggle="tab" data-bs-target="#pending" type="button" role="tab">
                <i class="bi bi-clock me-2" style="color:#20798c;"></i>Pending Volunteers ({{ $pendingVolunteers->count() }})
                @if($pendingVolunteers->count() > 0)
                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-warning">
                        {{ $pendingVolunteers->count() }}
                    </span>
                @endif
            </button>
        </li>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    <div class="avatar-sm bg-primary text-white rounded-circle d-flex align-items-center justify-content-center me-3">
                                                        {{ strtoupper(substr($registration->user->getFullNameAttribute(), 0, 1)) }}
                                                    </div>
                                                    {{ $registration->user->getFullNameAttribute() }}
                                                </div>
                                            </td>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="btn-group" role="group">
                                                    <form method="POST" action="{{ route('staff.events.volunteers.approve', [$event, $registration]) }}" class="d-inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" class="btn btn-sm btn-success" onclick="return confirm('Approve this volunteer request?')">
                                                            <i class="bi bi-check-lg"></i> Approve
                                                        </button>
                                                    </form>
                                                    <form method="POST" action="{{ route('staff.events.volunteers.decline', [$event, $registration]) }}" class="d-inline">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Decline this volunteer request?')">
